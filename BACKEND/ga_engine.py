...
class GeneticRecommender:
    ...
    def calculate_bh_score(self, user_id, product_id, chromosome):
        # Calculate Behavioral score using Behavior DB
...
def main():
    ...
    if not all([db_users.connect(), db_products.connect(), 
        ...
        print("\nFailed to connect to all databases.")
        return
    ...
    best_chromosome = ga.run(generations=5)
    
    # Interactive Lookup
    print("Enter a user ID to see their top-10 recommendations.")
    print("Type 'q' to quit.\n")
    recommendation_cache = {}  # Initialize cache
    while True:
        user_input = input("User ID: ").strip()
        if user_input.lower() == 'q':
            break
        
        # Convert to expected ID type
        try:
        ...
        if user_id in recommendation_cache:
            ...
            # Compute recommendations for this user only
            recommendations = ga.get_recommendations(user_id, n=10)
        ...
        for rank, (prod_id, score) in enumerate(recommendations, 1):
            prod_info = preprocessor.product_info.get(prod_id, {})
            cat = prod_info.get('category', 'N/A')
            price = prod_info.get('price', '?')
            print(f"  {rank:2d}. Product {prod_id:4d} | {cat:15s} | Price: {price:5.0f}")
